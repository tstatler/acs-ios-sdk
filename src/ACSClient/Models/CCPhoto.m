...
-(id)initWithJsonResponse:(NSDictionary *)jsonResponse {
	...
	if ((self = [super initWithJsonResponse:jsonResponse])) {
        ...
        if (takenAt) {
            self.takenAt = [[[ACSClient defaultACSClient] jsonDateFormatter] dateFromString:takenAt];
        }
		...
		if (dateString) {
			self.customDate = [[[ACSClient defaultACSClient] jsonDateFormatter] dateFromString:dateString];
		}
        
        _exif = [[CCExif alloc] initWithJsonResponse:[jsonResponse objectForKey:@"exif"]];
        
	/*	if (self.processed == NO && [[ACSClient defaultACSClient] downloadManagerEnabled]) {
			// Photo hasn't been processed on the server, add to the download manager queue 
			// it will pull for its status periodically.
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handlePhotoProcessed:) name:@"PhotosProcessed" object:[ACSClient defaultACSClient]];
			[[ACSClient defaultACSClient].downloadManager addProcessingPhoto:self parent:nil];
		}*/
...
-(UIImage *)getImage:(PhotoSize)photoSize
{
    ...
    if (!image) {
        [[ACSClient defaultACSClient].downloadManager downloadPhoto:self size:[CCPhoto getPhotoSizeString:photoSize]];
...
-(NSString *)localPath:(PhotoSize)photoSize
{
	...
	return [NSString stringWithFormat:@"%@/%@_%d", [ACSClient defaultACSClient].acsDir, self.objectId, photoSize];
}
...
-(UIImage *)getImageForPhotoSize:(NSString *)photoSize
{
    ...
    if (!image) {
        [[ACSClient defaultACSClient].downloadManager downloadPhoto:self size:photoSize];
        // try again if the image was just downloaded
...
-(id)initWithJsonResponse:(NSDictionary *)jsonResponse
{
    ...
    if (self) {
		...
		if (dateString) {
			self.createDate = [[[ACSClient defaultACSClient] exifDateFormatter] dateFromString:dateString];
		}
