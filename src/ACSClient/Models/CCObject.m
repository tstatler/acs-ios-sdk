...
-(id)initWithJsonResponse:(NSDictionary *)jsonResponse
{
	...
	if (dateString) {
		self.createdAt = [[[ACSClient defaultACSClient] jsonDateFormatter] dateFromString:dateString];
	}
	
	dateString = [jsonResponse objectForKey:CC_JSON_UPDATED_AT];
	if (dateString) {
		self.updatedAt = [[[ACSClient defaultACSClient] jsonDateFormatter] dateFromString:dateString];
	}
