...
static ACSClient *theDefaultACSClient = nil;
...
void CCLog(NSString *format, ...) {
    if (theDefaultACSClient.loggingEnabled) {
        va_list arglist;
...
@interface ACSClient (PrivateMethods)
...
-(id)initWithAppKey:(NSString *)appKey customAppIds:(NSDictionary *)customAppIds
{
	if (appKey == nil || [appKey length] == 0) {
		[NSException raise:@"Missing ACS App Key" format:@"App Key is missing"];
	}
	if ((self = [super init])) {
		_appKey = [appKey copy];
...
-(id)initWithOauthConsumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret customAppIds:(NSDictionary *)customAppIds
{
	if ([consumerKey length] == 0 || [consumerSecret length] == 0) {
		[NSException raise:@"Missing ACS Oauth Consumer Key and/or Consumer Secret" format:@"Oauth info is missing"];
	}
	if ((self = [super init])) {
...
-(void)initCommon:(NSDictionary *)customAppIds
{
    ...
    theDefaultACSClient = self;
    self.apiURL = CC_BACKEND_URL;

	// create ACS dir if there is none
	NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES); 
	NSString *documentsDirectory = [paths objectAtIndex:0]; // Get documents folder
	_acsDir = [[NSString alloc] initWithFormat:@"%@/ACSCache", documentsDirectory];
	//[self cleanupCacheDir];

	if (![[NSFileManager defaultManager] createDirectoryAtPath:_acsDir withIntermediateDirectories:NO attributes:nil error:nil]) {
		CCLog(@"Failed to create %@, photo download will not work", _acsDir);
	}
	...
	if (customAppIds != nil) {
		...
		if (customAppId != nil) {
			...
			CCLog(@"ACS: initialized facebook with app Id %@", customAppId);
		}
	}
...
- (void)fbDidLogin {
	...
	if (_currentUser == nil) {
		// Failed to register with the ACS server
        //	[_facebook logout:self];
...
-(void)fbDidLogout {
	// Logout has to go through ACS server
}
...
-(void)saveUserSession
{
	...
	// debug
    if ([[ACSClient defaultACSClient] loggingEnabled]) {
...
/*-(void)cleanupCacheDir
{
	[[NSFileManager defaultManager] removeItemAtPath:_acsDir error:nil];
}*/
...
-(void)ccrequest:(CCRequest *)request didSucceed:(CCResponse *)response
{
    ...
    if ([users count] == 1) {
        ...
        [[ACSClient defaultACSClient] setCurrentUser:user];
    }
}
...
-(void)ccrequest:(CCRequest *)request didFailWithError:(NSError *)error
{
    ...
    [[ACSClient defaultACSClient] setCurrentUser:nil];
}*/

+(void)initializeWithAppKey:(NSString *)appKey customAppIds:(NSDictionary *)customAppIds
{
    @synchronized(theDefaultACSClient) {
        if (theDefaultACSClient != nil) {
            return;
        }
        theDefaultACSClient = [[ACSClient alloc] initWithAppKey:appKey customAppIds:customAppIds];
    }
}
...
+(void)initializeWithOauthConsumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret customAppIds:(NSDictionary *)customAppIds
{
    @synchronized(theDefaultACSClient) {
        ...
        theDefaultACSClient = [[ACSClient alloc] initWithOauthConsumerKey:consumerKey consumerSecret:consumerSecret customAppIds:customAppIds];
    }
}
...
+(ACSClient *)defaultACSClient
{
    @synchronized(theDefaultACSClient) {
        if (theDefaultACSClient == nil) {
            [NSException raise:@"Uninitialized" format:@"Use [ACSClient initializeWithAppId:customAppIds:] to initialize ACS"];
        }
        return theDefaultACSClient;
    }
}
