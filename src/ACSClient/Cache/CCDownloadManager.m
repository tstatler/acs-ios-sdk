...
-(void)downloadDone:(CCRequest *)request
{
	...
	NSNotification * myNotification = [NSNotification notificationWithName:@"DownloadFinished" object:[ACSClient defaultACSClient] userInfo:request.userInfo];
...
{
	NSNotification * myNotification = [NSNotification notificationWithName:@"DownloadFailed" object:[ACSClient defaultACSClient] userInfo:request.userInfo];
	[[NSNotificationQueue defaultQueue] enqueueNotification:myNotification postingStyle:NSPostNow];	
} 
...
-(void)ccrequest:(CCRequest *)request didSucceed:(CCResponse *)response
{
	...
	if ([processedPhotos count] > 0) {
		...
		NSNotification * myNotification = [NSNotification notificationWithName:@"PhotosProcessed" object:[ACSClient defaultACSClient] userInfo:dict];
		[[NSNotificationQueue defaultQueue] enqueueNotification:myNotification postingStyle:NSPostNow];	
	}
...
-(void)cleanupCache
{
    @synchronized(self) {
        ...
        // get it on IOS
        NSError* error = nil;
        NSArray* filesArray = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[ACSClient defaultACSClient].acsDir error:&error];
        if(error != nil) {
        ...
        for(NSString* file in filesArray) {
            NSString* filePath = [[ACSClient defaultACSClient].acsDir stringByAppendingPathComponent:file];
